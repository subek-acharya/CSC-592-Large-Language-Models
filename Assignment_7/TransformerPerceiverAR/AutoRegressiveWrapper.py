...
class AutoRegressiveWrapper(nn.Module):
    ...
    def forward(self, x):
        #assert(x.shape[-1] == 1025),'seq length should be 1024'
        xi = x[:, :-1] # x is input of size seq_len+1, :-1 will make it seq_le 
        xo = x[:, -self.latent_len:]  # expected output in training is shifted one to the left
        out = self.model(xi)
